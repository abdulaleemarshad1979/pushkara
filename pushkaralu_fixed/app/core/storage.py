...
async def upload_image(file: Optional[UploadFile], folder: str = "lost-found") -> Optional[str]:
    ...
    try:
        ...
        # ── FIX (Issue 6): Hardened filename sanitisation ─────────────────────
        # PROBLEM: The old code only stripped path separators. An attacker can
        # still inject null bytes (\x00), shell metacharacters, or control
        # characters that may cause issues on certain filesystems or web servers
        # serving the uploads/ directory in the local fallback path.
        #
        # SOLUTION: Use a strict allowlist — keep only alphanumeric chars, dots,
        # hyphens, and underscores. Verify the extension matches the detected
        # MIME type. Prepend a UUID regardless (collision-proof, no guessable path).
        import re
        import unicodedata
        ...
        # Extract extension BEFORE allowlist stripping
        _, dot_ext = os.path.splitext(os.path.basename(raw_name))
        dot_ext = dot_ext.lower()[:8]   # cap at 8 chars; guard against .php5.jpg tricks

        # Validate extension matches the detected MIME type
        _ALLOWED_IMAGE_EXTS = {".jpg", ".jpeg", ".png", ".gif", ".webp", ".bmp", ".avif"}
        if dot_ext not in _ALLOWED_IMAGE_EXTS:
            logger.info("[Storage] Rejected — extension %s not in allowlist", dot_ext)
            return None

        # Strip everything except safe chars for the stem
        stem = re.sub(r"[^a-zA-Z0-9._-]", "_", os.path.splitext(os.path.basename(raw_name))[0])
        stem = stem[:64]   # max stem length to prevent very long keys
        safe_name = f"{stem}{dot_ext}"
