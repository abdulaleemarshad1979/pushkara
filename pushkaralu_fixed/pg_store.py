# DEPRECATED — use app/core/pg_store.py instead. This root-level copy is no longer imported.
    ...
    try:
        async with pool.acquire() as conn:
            ...
            async with conn.transaction(readonly=True):
                ...
                if rows:
                    db["sos_alerts"] = [json.loads(r["payload"]) for r in rows]
                ...
                if rows:
                    db["issues"] = [json.loads(r["payload"]) for r in rows]
                ...
                if rows:
                    db["lost_persons"] = [json.loads(r["payload"]) for r in rows]
